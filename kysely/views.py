...
class ListaNäkymä(generic.ListView):
    ...
    def get_queryset(self):
        ...
        #Suodatetaan (filter) kaikista kysymyksistä ne, joiden julkaisupvm on pienempi tai yhtäsuuri kuin tämänhetkinen 
        #aika ("muuttujassa nyt")
        #HUOM! Ite = Less Than or Equal = pienempi tai yhtäsuuri
        ei_tulevaisuudessa = kaikki_kysymykset.filter(julkaisupvm__Ite=nyt)

        #Järestetään kysymykset julkaisupvm:n mukaan 
        #HUOM! "-" -merkki edessä kääntää järjestyksen niin, että suuret arvot tulevat ennen pieniä, jolloin uusimmat 
